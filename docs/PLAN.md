...
| D-033 | 2026-09-27 | Return odds are calibrated out of time and computed at deploy. services/return_odds.py builds the table (one implementation for backend and pipeline) and scales it by actual over predicted returns, pooled over the last three yearly test points; pipeline.hazard.add_return_odds writes the calibrated table into arrival_hazard.json, and the backend falls back to the uncalibrated table only if it is missing. A publisher factor is not used. Chances under 1% read "fewer than 1 in 100" / "Under 1%". | Owner asked whether the 4% for Persona 3 Reload could be narrowed by publisher. Tested at three yearly cut-offs: a shrunk publisher factor made predictions worse on Game Pass and PS Plus at every strength and was no better on Epic. The same test showed the table itself too high: for games 0-2 years since their last run it predicted about 5% and 1.8% (Game Pass), 2.2% (PS Plus) and 3.4% (Epic) actually returned. Single-year ratios are lumpy (0.2x to 4x), so three years are pooled. Calibrated, nearly every game reads "Unlikely to return" with a 1-3% chance, which is what the data supports. Computing the calibration at start-up took about 30 s, so it runs at deploy; start-up is 5 s. | refines D-032 |
